...
const LaporanPetugas = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8081/laporan-petugas')
            .then(res => {
                        ...
                        $(tableRef.current).DataTable({
                            ...
                            columns: [
                                ...
                                { title: 'Detail', render: function (data, type, row, meta) {
                                    ...
                                    const tahun = row.tahun; // replace 'tahun' with the actual field name from your data
                                    return `
                                        <a href="/tahun-petugas/${id}" class='btn btn-outline-success btn-block btn-flat'>Open</a>
                                    `;
                                },},
                                {
                                    title: 'Aksi',
                            ...
                            createdRow: function (row, data, dataIndex) {
                                ...
                                const statusCell = $('td:eq(2)', row); // Change 4 to the correct index of the "Status" column
                                    statusCell.css('color', '#4FAC16'); // Set text color to green
                                    statusCell.html(`<a href="/tahun-petugas/${data.ID}"><span class="bg-[#F9E3D0] text-[#FF9130] px-4 py-1 rounded-full" style="width: 120px; display: inline-block;">Lihat Detail</span></a>`);
                                },
                        });
                        const searchInput = $(tableRef.current).closest('.dataTables_wrapper').find('input[type="search"]');
                        searchInput.css('margin-bottom', '10px'); // Adjust the margin as needed
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="flex flex-col gap-5">
                            ...
                            <div className="flex justify-end gap-3">
                                <button className="bg-gray-500 text-[#FFFFFF] text-sm font-medium py-2 px-3 rounded-md" onClick={() => { return false }}>Cancel</button>
